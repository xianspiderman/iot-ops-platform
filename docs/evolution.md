...
## v1.0.0 — access control, operability and public delivery

### Case 7: direct permission lookup to Redis cache and session control

**Business problem -> first version.** v0.1.0—v0.3.0 used Sa-Token only to establish an authenticated user. That was sufficient to keep early business iterations executable, but every signed-in user could reach every protected business endpoint and there was no project-level data boundary. The smallest RBAC implementation would join user-role-permission on every request. It is correct and became the cache miss path, but stable authorization relationships make repeated joins unnecessary.

**Exposed problem -> candidate comparison.** Three caching locations were compared. A JVM cache is fastest but creates instance-local invalidation; permissions embedded in a long-lived token reduce lookups but make revocation depend on token expiry or a blacklist; Redis cache-aside can share state and retain database fallback, at the cost of explicit invalidation. Redis was already an intentional platform dependency, so cache-aside was chosen. Data scope remains a separate database query because it participates in domain filtering and must not be confused with endpoint permission codes.

**Final choice -> implementation order.** Flyway V4 first adds normalized user-role, role-permission and user-project-scope relations plus audit records. Sa-Token's permission provider then loads a per-user JSON array from Redis under a 30-minute TTL, or queries MySQL and best-effort fills the cache. Controllers declare permission codes; services call `DataScopeService` before project-scoped reads and writes. The Vue session store obtains the same permission set from `/auth/me` to hide unavailable navigation, while the backend remains the enforcement point.

**Concurrent change and failure paths.** Permission mutation determines all affected users, deletes their caches before the database transaction, changes relations and writes an audit row, then registers an after-commit deletion and logs out their sessions. The first deletion narrows the old-value window; the second removes an old value that a concurrent request may have repopulated during the transaction. Logout forces the next request to authenticate again instead of silently continuing with revoked authority. Redis read or write failure falls back to MySQL, so cache availability does not become authentication availability. A database rollback does not publish new permission state.

**Test evidence -> advantages and cost.** A Redis 8.2.1 and MySQL 8.4.11 Testcontainer test observes one miss followed by one hit, replaces the operator role's permissions, verifies the prior token no longer maps to a login ID, then observes the new database permission after cache reload and an audit row. A second test creates project 2 and proves the operator only lists project 1 devices and cannot authorize a project 2 write. Full Compose HTTP verification observes 16 admin permissions, project 1 operator scope and HTTP 403 for operator access to RBAC.

The design reduces repeated role joins and gives revocation an explicit behavior, but each authorization mutation has more invalidation work and affected-user discovery. Current deployment has one backend instance. Redis Pub/Sub or an authorization event will be considered only if multi-instance testing shows a measurable cross-node invalidation delay; permission-in-token will not be adopted while immediate revocation is required.

### Completing the operational surface
